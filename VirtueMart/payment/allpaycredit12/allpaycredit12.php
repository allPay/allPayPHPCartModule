...
class plgVmPaymentAllPayCredit12 extends vmPSPlugin {
    ...
    function plgVmOnShowOrderBEPayment($virtuemart_order_id, $virtuemart_payment_id) {
        ...
        //$html .= $this->getHtmlRowBE('CREDIT12_PAYMENT_NAME', $paymentTable->payment_name);
        //$html .= $this->getHtmlRowBE('CREDIT12_PAYMENT_TOTAL_CURRENCY', $paymentTable->payment_order_total . ' ' . $paymentTable->payment_currency);
        //$html .= '</table>' . "\n";
        return $html;
    }
        ...
        if ($order) {
            try {
                ...
                array_push($oPayment->Send['Items'], array(
                    'Name' => vmText::_('VMPAYMENT_CREDIT12_ORDER_INFO'),
                ...
                $szHtml .= $this->getHtmlRow('CREDIT12_PAYMENT_METHOD', $this->renderPluginName($this->_currentMethod), "vmorder-done-payinfo");
                $szHtml .= $this->getHtmlRow('CREDIT12_ORDER_NUMBER', $order['details']['BT']->order_number, "vmorder-done-nr");
                $szHtml .= $this->getHtmlRow('CREDIT12_ORDER_TOTAL', $oCurrency->priceDisplay($order['details']['BT']->order_total), "vmorder-done-amount");
                $szHtml .= $this->getHtmlRow('CREDIT12_RESPONSE', vmText::_('VMPAYMENT_CREDIT12_RESPONSE_PROCESSING') . $oPayment->CheckOutString('結帳'));
                $szHtml .= '</table>' . "\n";
    ...
    function plgVmOnPaymentResponseReceived(&$html) {
        ...
        try {
            ...
            if (sizeof($arFeedback) > 0) {
                ...
                if ($deTradeAmount == $deTotalAmount) {
                    ...
                    if ($szReturnCode == 1 || $szReturnCode == 800) {
                        $szComment = vmText::_('VMPAYMENT_CREDIT12_RESPONSE_SUCCESS');
                        // 更新訂單資料與新增訂單歷程。
                        ...
                        $html .= $this->getHtmlRow('CREDIT12_PAYMENT_INFO', $this->renderPluginName($this->_currentMethod), "vmorder-done-payinfo");
                        $html .= $this->getHtmlRow('CREDIT12_ORDER_NUMBER', $szOrderID, "vmorder-done-nr");
                        $html .= $this->getHtmlRow('CREDIT12_ORDER_TOTAL', $oCurrency->priceDisplay($oOrder['details']['BT']->order_total), "vmorder-done-amount");
                        $html .= $this->getHtmlRow('CREDIT12_RESPONSE', $szComment . ' ' . vmText::_('VMPAYMENT_CREDIT12_RESPONSE_PENDING'));
                        $html .= '</table>' . "\n";
                    } else {
            ...
            $html = '<table class="vmorder-done">' . "\n";
            $html .= $this->getHtmlRow('CREDIT12_PAYMENT_INFO', $this->renderPluginName($this->_currentMethod), "vmorder-done-payinfo");
            $html .= $this->getHtmlRow('CREDIT12_ORDER_NUMBER', $szOrderNumber, "vmorder-done-nr");
            $html .= $this->getHtmlRow('CREDIT12_RESPONSE', vmText::_('VMPAYMENT_CREDIT12_RESPONSE_FAIL'));
            $html .= '</table>' . "\n";
            // 顯示錯誤
            vmError($html);
        }
