...
// 電子發票載具類別
abstract class AllpayCarruerType
{
	// 無載具
	const None = '';
...
// 電子發票列印註記
abstract class AllpayPrintMark
{
	// 不列印
	const No = '0';
...
// 通關方式
abstract class AllpayClearanceMark
{
	// 經海關出口
	const Yes = '1';
...
// 延遲註記
abstract class AllpayDelayFlagType
{
	// 延遲註記
	const Delay = '1';
...
// 交易類別
abstract class AllpayPayTypeCategory
{
	// ECBANK
	const Ecbank = '1';
...
// 通知類別 
abstract class AllpayAllowanceNotifyType
{
	// 簡訊通知
	const Sms = 'S';
...
// 發送方式
abstract class AllpayNotifyType
{
	// 簡訊通知
	const Sms = 'S';
...
// 發送內容類型
abstract class AllpayInvoiceTagType
{
	// 發票開立
	const Invoice = 'I';
...
{	
	...
	function __construct()
	{
	        ...
	        $this->Send = array(
	            ...
	            "CustomerEmail" => '',
	            "ClearanceMark" => '',
	            "Print" => AllpayPrintMark::No,
	            "Donation" => AllpayDonation::No,
	            "LoveCode" => '',
		    "CarruerType" => AllpayCarruerType::None,
	            "CarruerNum" => '',
	            "TaxType" => '',
	            "SalesAmount" => '',
	            "InvoiceRemark" => '',
	...
	*/
		...
		// 檢查 A.一般開立發票、B.延遲觸發開立發票
	        ...
	        {		        	
	        	...
	        	if ($this->Send['Print'] == AllpayPrintMark::Yes)
        		{
			...
			if ($this->Send['TaxType'] == AllpayTaxType::Zero)
			{
				if ( ( $this->Send['ClearanceMark'] != AllpayClearanceMark::Yes ) && ( $this->Send['ClearanceMark'] != AllpayClearanceMark::No ) )
				{
			...
			else
			{
	        	...
	        	// *列印註記僅能為 0 或 1
			if ( ( $this->Send['Print'] != AllpayPrintMark::Yes ) && ( $this->Send['Print'] != AllpayPrintMark::No ) )
			{
				array_push($arErrors, "12:Invalid Print.");
			}
			// *若捐贈註記 = '1' (捐贈)時，則VAL = '0' (不列印)
			if ($this->Send['Donation'] == AllpayDonation::Yes)
			{
				if ($this->Send['Print'] != AllpayPrintMark::No)
				{
			...
			if ( ($this->Send['Donation'] != AllpayDonation::Yes ) && ( $this->Send['Donation'] != AllpayDonation::No ) )
			{
				array_push($arErrors, "13:Invalid Donation.");
			}
			// *若統一編號有值時，則VAL = '2' (不捐贈)
			if (strlen($this->Send['CustomerIdentifier']) > 0 && $this->Send['Donation'] == AllpayDonation::Yes )
			{
			...
			if ( ( $this->Send['CarruerType'] != AllpayCarruerType::None ) && ( $this->Send['CarruerType'] != AllpayCarruerType::Member ) && ( $this->Send['CarruerType'] != AllpayCarruerType::Citizen ) && ( $this->Send['CarruerType'] != AllpayCarruerType::Cellphone ) )
			{
			...
			else
			{
				...
				if (strlen($this->Send['CustomerIdentifier']) > 0)
				{
					if ($this->Send['CarruerType'] == AllpayCarruerType::Member || $this->Send['CarruerType'] == AllpayCarruerType::Citizen )
					{
                          			array_push($arErrors, "15:Invalid CarruerType.");
                      			}		
				}
			...
			switch ($this->Send['CarruerType'])
			{
              			...
              			case AllpayCarruerType::None:
              			case AllpayCarruerType::Member:
				...
				case AllpayCarruerType::Citizen:
				...
				break;
              			...
              			case AllpayCarruerType::Cellphone:
			...
			if ( ( $this->Send['TaxType'] != AllpayTaxType::Dutiable ) && ( $this->Send['TaxType'] != AllpayTaxType::Zero ) && ( $this->Send['TaxType'] != AllpayTaxType::Free ) && ( $this->Send['TaxType'] != AllpayTaxType::Mix ) )
			{
		...
		// 檢查 A.一般開立發票、B.延遲觸發開立發票
			...
			if( ( $this->Send['InvType'] != AllpayInvType::General ) && ( $this->Send['InvType'] != AllpayInvType::Special ) )
			{
		...
		if( $this->Invoice_Method == InvoiceMethod::INVOICE )
			...
			{
				if( ( $this->Send['vat'] != AllpayVatType::Yes ) && ( $this->Send['vat'] != AllpayVatType::No ) )
				{
		...
		// 檢查 B.延遲觸發開立發票
	        ...
	        {
	        	// 30.延遲註記 DelayFlag
			if( ( $this->Send['DelayFlag'] != AllpayDelayFlagType::Delay ) && ( $this->Send['DelayFlag'] != AllpayDelayFlagType::Trigger ) )
			{
			...
			if ( $this->Send['DelayFlag'] == AllpayDelayFlagType::Delay )
			{
			...
			if ($this->Send['DelayFlag'] == AllpayDelayFlagType::Trigger)
			{
		...
		// 檢查 B.延遲觸發開立發票、K.付款完成觸發或延遲開立發票
			...
			if( ( $this->Send['PayType'] != AllpayPayTypeCategory::Ecbank ) && ( $this->Send['PayType'] != AllpayPayTypeCategory::Ecpay ) && ( $this->Send['PayType'] != AllpayPayTypeCategory::Allpay ) )
			{
		...
		// 檢查 B.延遲觸發開立發票
			...
			switch ($this->Send['PayType'])
			{
              			...
              			case AllpayPayTypeCategory::Ecbank:
					$this->Send['PayAct'] = 'ECBANK' ;
				break;
                 		 
				// *若交易類別='2'時，則VAL = 'ECPAY'
              			case AllpayPayTypeCategory::Ecpay:
					$this->Send['PayAct'] = 'ECPAY' ;
				break;
              
              			// *若交易類別='3'時，則VAL = 'ALLPAY'
              			case AllpayPayTypeCategory::Allpay:
	        ...
	        {	
	        	...
	        	// *固定給定下述預設值
	        	if( ( $this->Send['AllowanceNotify'] != AllpayAllowanceNotifyType::Sms ) && ( $this->Send['AllowanceNotify'] != AllpayAllowanceNotifyType::Email ) && ( $this->Send['AllowanceNotify'] != AllpayAllowanceNotifyType::All ) && ( $this->Send['AllowanceNotify'] != AllpayAllowanceNotifyType::None ) )
			{
			...
			if(strlen($this->Send['NotifyMail']) > 0 )
	        	...
	        	if($this->Send['AllowanceNotify'] == AllpayAllowanceNotifyType::Email && strlen($this->Send['NotifyMail']) == 0 )
	        	{
	        	...
	        	if (strlen($this->Send['NotifyPhone']) > 20)
			{
	        	...
	        	if( $this->Send['AllowanceNotify'] == AllpayAllowanceNotifyType::Sms && strlen($this->Send['NotifyPhone']) == 0 )
	        	{
	        		array_push($arErrors, "40:NotifyPhone is required.");
	        	}
	        	
	        	// 39-40 通知電子信箱、通知手機號碼不能全為空值 (如果狀態為SMS 或 EMAIL)
	        	if(strlen($this->Send['NotifyPhone']) == 0 && strlen($this->Send['NotifyMail']) == 0 && ( $this->Send['AllowanceNotify'] == AllpayAllowanceNotifyType::Sms || $this->Send['AllowanceNotify'] == AllpayAllowanceNotifyType::Email ) )
	        	{
	        	...
	        	else
	        	{
		        	...
		        	if( $this->Send['AllowanceNotify'] == AllpayAllowanceNotifyType::All && ( strlen($this->Send['NotifyMail']) == 0 || strlen($this->Send['NotifyPhone']) == 0 ) )
		        	{
		        		array_push($arErrors, "39-40:NotifyMail And NotifyPhone is required.");
		        	}
		        	
		        	// *下述情況通知手機號碼與電子信箱為空值(通知類別為N-皆不通知)
		        	if($this->Send['AllowanceNotify'] == AllpayAllowanceNotifyType::None && ( strlen($this->Send['NotifyMail']) > 0 || strlen($this->Send['NotifyPhone']) > 0 ))
		        	{
		...
		// 檢查J.發送通知
			...
			// *固定給定下述預設值
			if( ( $this->Send['InvoiceTag'] != AllpayInvoiceTagType::Invoice ) && ( $this->Send['InvoiceTag'] != AllpayInvoiceTagType::Invoice_Void ) && ( $this->Send['InvoiceTag'] != AllpayInvoiceTagType::Allowance ) && ( $this->Send['InvoiceTag'] != AllpayInvoiceTagType::Allowance_Void ) && ( $this->Send['InvoiceTag'] != AllpayInvoiceTagType::Invoice_Winning ) )
	        	{
			...
			// *固定給定下述預設值
			if( ( $this->Send['Notified'] != AllpayNotifiedType::Customer ) && ( $this->Send['Notified'] != AllpayNotifiedType::vendor ) && ( $this->Send['Notified'] != AllpayNotifiedType::All ) )
	        	{
